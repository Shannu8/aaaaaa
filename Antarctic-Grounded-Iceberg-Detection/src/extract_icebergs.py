...
# ==========================================
def apply_coastline_mask(target_tif, output_tif, mask_gpkg, dilation_pixels=40, coastline_layer="add_coastline_high_res_polygon_v7_11"):
    ...
    try:
        ...
        gdf = gpd.read_file(mask_gpkg, layer=coastline_layer)
        mask_gdf = gdf[gdf['surface'].isin(mask_values)].copy()

        if mask_gdf.empty:
            print(f"Warning: No land or ice shelf features found in {mask_gpkg}. Skipping coastline mask.")
            import shutil
            shutil.copy2(target_tif, output_tif)
            return True
...
def process_iceberg_extraction_block(
    base_mask_folder, timestamps, output_folder, block_rows, block_cols, min_area_threshold, prefix="raw_"
):
    ...
    for timestamp in timestamps:
        ...
        out_mask = os.path.join(output_folder, f"{prefix}labeled_mask_{timestamp}.tif")
        out_json = os.path.join(output_folder, f"{prefix}regions_{timestamp}.json")
...
def main():
    ...
    parser.add_argument("--coastline_layer", type=str, default="add_coastline_high_res_polygon_v7_11", help="(Optional) Layer name within the coastline GPKG file")
    ...
    if args.coastline_gpkg and os.path.exists(args.coastline_gpkg):
        ...
        for tif in tif_files:
            ...
            if not out_tif.exists():
                apply_coastline_mask(tif, out_tif, args.coastline_gpkg, coastline_layer=args.coastline_layer)
    
    # Step 2: Extract Connected Regions
    print(f"Initiating connected region extraction process (Mode: {prefix.strip('_')})...")
    timestamps = []
